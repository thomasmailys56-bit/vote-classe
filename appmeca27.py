...
try:
    ...
    # Logique de la question par date
    date_aujourdhui = datetime.now().strftime("%d/%m/%Y")
    # On cherche la question dont la colonne 'Date' correspond à aujourd'hui
    ...
    if not q_du_jour.empty:
        ...
        question_actuelle = "Pas de question prévue pour aujourd'hui ! 😴"
...
if 'user' not in st.session_state:
    ...
    with st.container(border=True):
        if mode == "Connexion":
            ...
            if st.button("Créer mon compte ✨", use_container_width=True):
                if new_nom in liste_noms:
                    st.error("Ce nom existe déjà !")
                elif new_mdp != confirm:
                    st.error("Les mots de passe diffèrent.")
                elif new_nom and new_mdp:
    ...
    with tab1:
        ...
        # Filtrer les votes pour la question du jour (par date)
        # On suppose que l'onglet Votes pourrait avoir une colonne 'Date' pour séparer les jours
        # Pour simplifier, on reset l'onglet Vote quand une nouvelle question arrive.
        deja_vote = st.session_state.user in df_votes["Votant"].astype(str).values if not df_votes.empty else False

        if not deja_vote and question_actuelle != "Pas de question prévue pour aujourd'hui ! 😴":
            with st.container(border=True):
                cible = st.radio("Ton choix :", liste_noms, horizontal=True)
                if st.button("Voter", use_container_width=True):
            ...
            if not df_votes.empty:
                st.success("Résultats en temps réel :")
    ...
    # --- 6. SECTION ADMIN PROGRAMMATION ---
    st.divider()
    st.caption(f"👑 Admin du jour : {admin_du_jour}")
    
    if st.session_state.user == admin_du_jour:
        with st.expander("🛠️ Programmer une question"):
            q_txt = st.text_input("Texte de la question :")
            q_date = st.date_input("Pour quel jour ?", min_value=date.today())
            
            if st.button("Enregistrer la question", use_container_width=True):
                if q_txt:
                    date_str = q_date.strftime("%d/%m/%Y")
                    # On nettoie si une question existe déjà pour ce jour précis
                    df_q_clean = df_q[df_q["Date"] != date_str]
                    nv_q = pd.DataFrame([{"Texte": q_txt, "Date": date_str, "Auteur": st.session_state.user}])
                    conn.update(worksheet="Question", data=pd.concat([df_q_clean, nv_q], ignore_index=True))
                    
                    # Si on programme pour AUJOURD'HUI, on vide les votes
                    if date_str == date_aujourdhui:
                        conn.update(worksheet="Votes", data=pd.DataFrame(columns=["Votant", "Cible"]))
                    
                    st.success(f"Question programmée pour le {date_str} !")
                    st.rerun()
